...
def kullaniciOturumAc(kadi, sifre):
    conn = mysql.connector.connect(
    host="localhost",
    user="root",
    password="",
    database="socialepvp_db"
    )

    # Verileri çekmek için bir cursor (imleç) oluşturma
    cursor = conn.cursor()
    
    # Sorguyu çalıştırma
    sql = "SELECT oyuncu_uid FROM oyuncu_tablo WHERE oyuncu_kullaniciAdi = %s AND oyuncu_kullaniciSifre = %s"
    vals = (kadi, rawTextSifreleme(sifre))
    cursor.execute(sql, vals)
    
    # Tüm verileri al
    veriler = cursor.fetchall()
    
    # Sütun verilerini liste olarak elde etme
    sutun_verileri = [veri[0] for veri in veriler]
    kulln = ""
    # Sütun verilerini yazdırma
    for veri in sutun_verileri:
        kulln = veri
    
    # Bağlantıyı ve cursor'ı kapatma
    cursor.close()
    conn.close()
    session['USERID'] = kulln
    session['GAMEVERSION'] = "SocialEmpires0926bsec.swf"


## PAGES AND RESOURCES
def rawTextSifreleme(content):
    # Metni MD5 ile şifreleme
    md5_hash = hashlib.md5()
    md5_hash.update(content.encode('utf-8'))
    encrypted_text = md5_hash.hexdigest()
    return encrypted_text

def girisKontrol(kadi, sifre):
    ...
    # Kullanıcı adına veya e-posta adresine göre kayıt olup olmadığını kontrol etme
    sql = "SELECT COUNT(*) FROM oyuncu_tablo WHERE oyuncu_kullaniciAdi = %s OR oyuncu_kullaniciSifre = %s"
    values = (kadi, sifre)
    ...
    # Kayıt varsa False, yoksa True döndürme
    if user_count > 0:
        kullaniciOturumAc(kadi, sifre)
        return True
    else:
        return False

def kullaniciKontrol(username, email):
    # Veritabanı bağlantısını oluşturma
    connection = mysql.connector.connect(
        host="localhost",
        user="root",
    ...
    # Kullanıcı adına veya e-posta adresine göre kayıt olup olmadığını kontrol etme
    sql = "SELECT COUNT(*) FROM oyuncu_tablo WHERE oyuncu_kullaniciAdi = %s OR oyuncu_Eposta = %s"
...
def kullaniciEkle(username, password, email, uid):
    # Veritabanı bağlantısını oluşturma
    connection = mysql.connector.connect(
        host="localhost",
        user="root",
    ...
    # Veri eklemek için bir cursor (imleç) oluşturma
    cursor = connection.cursor()

    # Kullanıcıyı kaydetme
    sql = "INSERT INTO oyuncu_tablo (oyuncu_KayitId, oyuncu_kullaniciAdi, oyuncu_kullaniciSifre, oyuncu_Eposta, oyuncu_uid) VALUES (%s, %s, %s, %s, %s)"
    values = (random.randint(1111111, 9999999), username, rawTextSifreleme(password), email, uid)
    cursor.execute(sql, values)
...
def anasayfa():
    if request.method == 'POST':
        if 'kayit_Ol' in request.form:
            if kullaniciKontrol(request.form["kullanici_KayitAdi"], request.form["kullanici_KayitEposta"]):
                sess = new_village(request.form["kullanici_KayitAdi"])
                kullaniciEkle(request.form["kullanici_KayitAdi"], request.form["kullanici_KayitSifre"], request.form["kullanici_KayitEposta"], sess)
                session['USERID'] = sess
                session['GAMEVERSION'] = "SocialEmpires0926bsec.swf"
                session['k'] = request.form["kullanici_KayitAdi"]
                return redirect("/play.html")
            else:
                return redirect("/")
            
            
        elif 'giris_Yap' in request.form:
            if girisKontrol(request.form["kullanici_Adi"], request.form["kullanici_Sifre"]):
                print("DOGUR!")
                return redirect("/play.html")
            else:
                print("NO!")
...
# @app.route("/translocation", methods=['GET', 'POST'])
# def login():
        ...
        if request.method == "POST":
            ...
            # Kayıt Olma
                ...
                if kullaniciKontrol(request.form["kullanici_KayitAdi"], request.form["kullanici_KayitEposta"]):
                    ...
                    return "NO"
        # İstenmeyen durumlar için varsayılan bir dönüş değeri
        return "Hatalı Api Parametresi!"
...
#    session['GAMEVERSION'] = "SocialEmpires0926bsec.swf"
#    print("New village fonksiyonu çıktısı: " + session['USERID'])
#    return redirect("play.html")
